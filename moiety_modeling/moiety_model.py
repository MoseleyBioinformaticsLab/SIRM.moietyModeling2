...
"""
moiety_model.py is used to create the moiety_model entity.
"""
...
class Moiety:
    """The description of the moiety entity."""

    def __init__(self, name, maxIsotopeNum, isotopeStates, nickname=' ', ranking=0):
        """

        :param name: the name of the moiety.
        :param maxIsotopeNum: the dictionary of the labeling isotopes and the corresponding number in the moiety. eg: {'13C': 5, '15N': 3}
        :param isotopeStates: the dictionary of the labeling isotopes and the corresponding states in the moiety. eg: {'13C': [0, 2, 5], '15N': [2, 3]}
        :param nickname: the nickname of the moiety.
        :param ranking: the ranking of the moiety.
        """
        self.name = name
...
class Relationship:
    """The description of relationship between the moietyStates."""

    def __init__(self, moiety, moietyState, equivalentMoiety, equivalentMoietyState, operator=None, coefficient=None):
        """

        :param moiety: the moiety in the relationship.
        :param moietyState: the state of the moiety (eg: '13C0.15N1').
        :param equivalentMoiety: the equivalent moiety in the relationship.
        :param equivalentMoietyState: the state of the equivalentMoiety (eg: '13C0.15N1')
        :param operator: the operator of the relationship.
        :param coefficient: the coefficient of the relationship.
        """
        self.moiety = moiety
...
class Molecule:
    """The description of the molecule entity."""

    def __init__(self, name, moieties):
        """
        :param name: the name of the molecule.
        :param moieties: the list of moiety objects that make up for the molecule.
        """
        self.name = name
...
class Model:
    ...
    def __init__(self, name, moieties, molecules, relationships=None):
            ...
            :param moieties: the list of the moiety objects.
            :param molecules: the list of the molecule objects.
            :param relationships: the list of the relationship objects.
            """
            self.name = name
            self.molecules = list(molecules)
            self.moieties = list(moieties)
            self.relationships = [] if relationships is None else list(relationships)
            self.relationshipDict = { relationship.varName: relationship for relationship in self.relationships }
    ...
    def _buildModel(self):
        ...
        for relationship in self.relationships:
            ...
            while testRelationship and testRelationship.equivalentVarName not in foundVarNames:
                ...
                testRelationship = self.relationshipDict[testRelationship.equivalentVarName] if testRelationship.equivalentVarName in self.relationshipDict else None
            if testRelationship:
        ...
        for moiety in self.orderedMoieties:
            ...
            for state in moiety.states:
                ...
                if varName in self.relationshipDict and self.relationshipDict[varName].equivalentMoiety in self.orderedMoieties:
                    numOfParameters -= 1
                    if self.relationshipDict[varName].moiety.name == self.relationshipDict[varName].equivalentMoiety.name:
                        # the relationship is appended to the postAssignedStates and preAssignedStates
                        postAssignedStates.append(self.relationshipDict[varName])
                        divisors[self.relationshipDict[varName].equivalentMoietyState] += self.relationshipDict[varName].multiplier
                    else:
                        preAssignedStates.append(self.relationshipDict[varName])
            for state in moiety.states:
                varName = '{0}[{1}]'.format(moiety.name, state)
                if varName not in self.relationshipDict or self.relationshipDict[varName].equivalentMoiety not in self.orderedMoieties:
    ...
    def calculateMoietyState(self, vector):
        ...
        :param vector: the list of the SAGA parameters of the model.
        ...
        for moiety in self.orderedMoieties:
            ...
            if properties['elements']:
                conversionDict[moiety.name + 'Parameters'] = [vector[i] for i in properties['elements']]
    ...
    def calculateModelEnergy(self, moietyStateValue, dataset):
